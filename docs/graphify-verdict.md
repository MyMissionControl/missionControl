...
- **Every 0.9.18 graph contained dangling edges** pointing at nodes that do not exist: 2.3% (`orches-skills`), 7.1% (`missionControl`), 12.5% (`maw-js`), 13.5% (`arra`) — and 8.9% even on a fresh 0.9.18 build. These materialized as attribute-free phantom nodes on load. **0.9.29 fixes this completely — measured 0 dangling edges in all six rebuilt graphs.** That fix accounts for almost the entire edge-count drop on rebuild: arra 34,923 → 29,764 edges, of which **4,711 were dangling**; maw-js 32,093 → 27,598, of which **4,002 were dangling**; node-id sets stayed identical (maw-js, maw-ui) or gained 2 (arra). The residual is a small genuine recall loss, not dedup: no duplicate `(source,target,relation)` triples existed in either version, and of arra's 546 dropped distinct edges **17 are `calls`**, of which 6 of 6 sampled are real calls verified in source (e.g. `useFirstRun` → `readFirstRunComplete`, `useFirstRun.ts:24/27`). Scale: live `calls` edges 6,239 → 6,224, i.e. **-0.24%**. Trading 0.24% of call recall for the elimination of 13.5% phantom edges is strongly net-positive, but it is a trade, not a free win.
